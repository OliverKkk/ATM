...
/*$query = 'select deposit_id from deposit';
...
if ($tcash >= 500)
{	
 $wdrawal="UPDATE tcash SET totalcash=(totalcash - 500)where bankname='$bank' and client_id=$id";
mysql_query($wdrawal,$conn) or die("Insertion Failed:" . mysql_error()); 
$insert="INSERT INTO deposit values('','$bank','$id',500,now())";
$rstt=mysql_query($insert) or die("Insertion Failed:" . mysql_error());
$insert="INSERT INTO withdraw values('','$bank','$id',500,now())";
$rstt=mysql_query($insert) or die("Insertion Failed:" . mysql_error());
