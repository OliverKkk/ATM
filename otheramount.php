...
$bank=$_SESSION[bank];
$amount=mysql_real_escape_string($_POST['otheramount']);
 ...
 echo "<script language='javascript' type='text/javascript'> window.alert('Some of the field are empty! Please fill it up.'); </script>";
		echo "<script language='javascript' type='text/javascript'> location.href='othera.php' </script>";
	}
	else{
				...
				do 
				{
				  ...
				  $wamount=$tcash-$amount;
				} 
				while ($myrow = mysql_fetch_array($result));
				if (($amount% 50) ==0)
				{
				if ($tcash > $amount){	
				    $wdrawal="UPDATE tcash SET totalcash=(totalcash - $amount)where bankname='$bank' and client_id=$id";
					...
					$insert="INSERT INTO withdraw values('','$bank','$id',$amount,now())";
					$rstt=mysql_query($insert) or die("Insertion Failed:" . mysql_error());
	...
	echo "<script language='javascript' type='text/javascript'> window.alert('Only amount accepted is 50,100,150,...!');  </script>";
						echo "<script language='javascript' type='text/javascript'> location.href='otheramount.php' </script>";	
				exit();		
				}
				}
